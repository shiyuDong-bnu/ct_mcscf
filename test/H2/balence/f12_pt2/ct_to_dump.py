...
mr_info = {}
mr_info["occupied_index"] = slice(0, 0)
mr_info["active_index"] = slice(0, 2)
mr_info["RDM1"] = psi4.core.variable("CURRENT RDM1").np
mr_info["RDM2"] = psi4.core.variable("CURRENT RDM2").np.reshape((2,) * 4)
...
## dump ct hamiltonian
def ct_reorder(hamiltonian, hf_ct):
...
h1e_ct, h2e_ct, cp_ct = ct_reorder(h_ct, rhf_ct)
