...
def calculate(basis):
    ...
    e_cas, mcscf_wfn = psi4.energy(
        "forte", forte_options=FORTE_OPTIONS,ref_wfn=wfn_hf, return_wfn=True
    )
    ...
    mr_info = {}
    mr_info["occupied_index"] = slice(0, 2)
    mr_info["active_index"] = slice(2, 5)
    mr_info["RDM1"] = psi4.core.variable("CURRENT RDM1").np
    mr_info["RDM2"] = psi4.core.variable("CURRENT RDM2").np.reshape((3,) * 4)
