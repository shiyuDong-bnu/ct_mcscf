...
import psi4
psi4.set_output_file("methylene_trip.out")
...
for bs in basis_sets:
    ...
    rohf_ct = rohf_energy(psi_mol, mcscf_wfn, h_ct)
    print("ct  hf energy is ", rohf_ct["escf"],
            " correlation energy is ", rohf_ct["escf"]-e_hf)
    h1e_ct, h2e_ct, cp_ct=convert_ct_to_quccsd(h_ct, rohf_ct)
    ...
    e_ct_scf,ct_mcscf_wfn=psi4.energy("forte",molecule=psi_mol,forte_options=forte_options,ref_wfn=mcscf_wfn,return_wfn=True)
    e_ct_casscf=psi4.core.variable('DSRG REFERENCE ENERGY')
    e_dsrg_pt=psi4.core.variable('CURRENT ENERGY')
    E_HF.append(e_hf)
    E_CT_HF.append(rohf_ct["escf"])
    ...
    print("cthf energy",rohf_ct["escf"])
